...
class DQN(Agent):
    def __init__(self, color, hyperparameters, training=True, number_of_nodes: int = 6, chain_length: int = 3):
        ...
        self.q_network = torch.nn.Sequential(
            torch.nn.Linear(number_of_nodes ** 2, hyperparameters['HIDDEN_LAYER_SIZE']),
            torch.nn.ReLU(),
            torch.nn.Linear(hyperparameters['HIDDEN_LAYER_SIZE'], hyperparameters['HIDDEN_LAYER_SIZE']),
            torch.nn.ReLU(),
            torch.nn.Linear(hyperparameters['HIDDEN_LAYER_SIZE'], number_of_nodes**2)
        )
        ...
        self.target_network = torch.nn.Sequential(
            torch.nn.Linear(number_of_nodes ** 2, self.hyperparameters['HIDDEN_LAYER_SIZE']),
            torch.nn.ReLU(),
            torch.nn.Linear(self.hyperparameters['HIDDEN_LAYER_SIZE'], self.hyperparameters['HIDDEN_LAYER_SIZE']),
            torch.nn.ReLU(),
            torch.nn.Linear(self.hyperparameters['HIDDEN_LAYER_SIZE'], number_of_nodes**2)
        )
    ...
    def update_q(self, state, action, reward, color=None):
        ...
        if len(self.experience_buffer) > self.hyperparameters['BATCH_SIZE']:
            ...
            current_qs = self.q_network(current_states)
            with torch.no_grad():
                new_qs_max = torch.max(self.target_network(new_states),1)
            for mem in sample:
                s, a, r, ns = mem
    ...
    def get_q(self, state, action):
        q_val = self.q_network(torch.flatten(Utils.weighted_adj(state, self.color)))[
    ...
    def hard_reset(self):
        self.reset()
        self.q_network = torch.nn.Sequential(
            torch.nn.Linear(self.number_of_nodes ** 2, self.hyperparameters['HIDDEN_LAYER_SIZE']),
            torch.nn.ReLU(),
            torch.nn.Linear(self.hyperparameters['HIDDEN_LAYER_SIZE'], self.hyperparameters['HIDDEN_LAYER_SIZE']),
            torch.nn.ReLU(),
            torch.nn.Linear(self.hyperparameters['HIDDEN_LAYER_SIZE'], self.number_of_nodes**2)
        )
